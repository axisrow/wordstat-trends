...
def _stability(out, emit, windows, vectors, args) -> int:
    ...
    emit()
    emit("**Причина:** бэкенды считают правдоподобие по разным формулам.")
    emit("statsforecast возвращает `−n/2·log(SSE)`; sktime (через statsmodels) —")
    emit("полное гауссово `−n/2·(log(2π) + log(SSE/n) + 1)`. Обе формулы проверены")
    emit("дословно на шести сочетаниях: первая совпала с loglik statsforecast с")
    emit("расхождением **0.00**, вторая — с loglik sktime с расхождением 0.01–3.9")
    emit("(остаток — разные оптимумы подгонки, не разные формулы).")
    emit()
    emit("Разность этих выражений раскладывается на два слагаемых **разного знака**,")
    emit("и это важно, потому что по одному только первому сдвиг получается втрое")
    emit("больше наблюдаемого:")
    emit()
    emit("| Слагаемое | При n = 56 |")
    emit("|---|---:|")
    emit("| опущенная аддитивная константа `−n/2·log(2π) − n/2` | −79.46 |")
    emit("| `SSE` не делится на `n`, отсюда `+n/2·log(n)` | +112.71 |")
    emit("| **итого ожидаемое Δloglik** | **+33.25** |")
    emit()
    emit("Предсказанное смещение loglik +33.25 (то есть sf ниже sk на 33.25) и сдвиг")
    emit("AIC −2·Δ = **+66.5** — против наблюдаемых Δloglik −29.3…−34.0 и ΔAIC ≈ +66.")
    emit("Сходится. Разброс по фразам возникает потому, что бэкенды приходят в чуть")
    emit("разные оптимумы, а не потому, что формула плавает.")
    emit()
